==========================================
3部 Pillowで「明日使えるスクリプト」を作る
==========================================

3部ではPillowを使って日常も使える便利なスクリプトを作っていきます。
2部までで学んだPythonの基本的な使い方、ライブラリーの使い方を活かして便利プログラムを完成させましょう。

3部で作るもの
=============

2部で画像の縮小スクリプト、画像のPNG=>JPEG変換スクリプトは作れました。
ですが2部のスクリプトでは毎度対象のファイル名を書き換える必要があったりして不便です:

* 対象のファイルを指定するためプログラムを修正する必要がある
* 一括変換できない

3部ではこの面倒な部分を直して、より日常使いやすいようなスクリプトにしましょう。
最初のステップとしてファイルを外部から指定できるようにします::

    $ python resize_image.py ./2016.png
    2016_thumb.png

次に指定したディレクトリー以下を探索して、指定された形式のすべての画像を変換するようにしましょう::

    $ python tree_resize_image.py ./images/ png
    ./images/2016_thumb.png
    ./images/2015_thumb.png
    ./images/2014_thumb.png

最後の目標としてすべてのスクリプトを1つのコマンドとしてまとめ、インストールできるようにしましょう::

    $ thumbnailer -s=0.5 ./2016.png
    ./images/2016_thumb.png

他にも余力があればLGTM画像メーカーなども作りましょう。

ステップ1: 引数でファイルを受け取る
===================================

``codes/3/resize_image.py`` を手元に写経しましょう。
...
例えばこの関数は今回取り組んでいる「画像の変換スクリプト」ではない題材でも汎用的に使える関数です。

関数に分離すると処理が見やすくなりますし、他のファイルで使いたくなった場合に再利用しやすくなります。
自分でモジュールやパッケージを作って再利用する方法は今回のチュートリアルでは扱いません。
...
標準ライブラリー ``argparse`` を使うとより高度な引数の指定が簡単に書けます。
...
``codes/3/tree_resize_image.py`` を手元に写経しましょう。
写経できたら以下のコマンドを実行して画像が生成されるか確認しましょう::
...
``codes/3/thumbnailer/`` 以下のPythonファイルを写経しましょう。
...
の順に書いていくと分かりやすいと思います。

インストール
-----------

``codes/3/thumbnailer`` ディレクトリー内 (``setup.py`` があるディレクトリー) で以下のコマンドを実行しましょう::
...
* ``main.py``: 引数の処理をしています。一番中心になる処理です。他の関数やライブラリーをインポートしてコマンドとして完成させています。

``main.py`` を見ると ``from .pathutil import dir_to_files`` とあります。
これは「同じ階層 (``.``) の ``pathutil`` から ``dir_to_files`` をインポート」という指定なので、
``main.py`` ファイル内で ``dir_to_files`` 関数が使えるようになっています。
この書き方は ``from PIL import Image`` としたのと同じですね。
ただインストールされたパッケージからインポートするのでなく、同階層の自分のPythonファイルからインポートしているのが違います。

Pythonファイルは中に入れる関数の意味ごとに分割しています。

argparseで引数の処理
^^^^^^^^^^^^^^^^^^^

``main.py`` 内の ``main`` 関数で ``argparse.ArgumentParser`` を使って引数の処理をしています。
pargparseを使うときの流れとしては
...
試してみたいことがあれば公式ドキュメントの `add_argument() メソッド <http://docs.python.jp/3/library/argparse.html#the-add-argument-method>`_ から探して使ってみましょう。
...
``argparse`` `サブコマンド <http://docs.python.jp/3/library/argparse.html#sub-commands>`_ を参考にすると良いでしょう。
